...
const options = {
	readOpts:          true,            //    --address-radix
	append:            true,            // -a --append
	ignore_interrupts: false,           // -i --ignore-interrupts
	output_error:      'warn-nopipe',   //    --output-error[=MODE]
};
			...
			for(let c of str){
				switch(c){
					...
					case 'i':
						...
						break;
					case 'p':
						options.output_error = 'warn-nopipe';
						break;
					default:
	...
	for(const file of fileList){
		fs.open(file, options.append ? 'a' : 'w', (err, fd) => {
			if(err){
				...
				if(options.output_error === 'exit' || options.output_error === 'exit-nopipe')
					process.exit(1);
				else
					process.exitCode = 1;
			}
...
const error = [
`\
${cmd}: ambiguous argument ‘’ for ‘--output-error’
Valid arguments are:
  - ‘warn’
  - ‘warn-nopipe’
  - ‘exit’
...
`\
${cmd}: invalid argument ‘_’ for ‘--output-error’
Valid arguments are:
  - ‘warn’
  - ‘warn-nopipe’
  - ‘exit’
