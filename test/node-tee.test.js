...
function loadTestData(){
	...
	cmdData.cmd_ext = 0;
	cmdData.cmd_desc = `${CMD_NODE_TEST} --help` + (cmdData.cmd_inp && ` << ${cmdData.cmd_inp}`);
	...
	cmdData.cmd_ext = 0;
	cmdData.cmd_desc = `${CMD_NODE_TEST} -h` + (cmdData.cmd_inp && ` << ${cmdData.cmd_inp}`);
	...
	cmdData.cmd_ext = 0;
	cmdData.cmd_desc = `NODE_TEE_TEST=1 ${CMD_NODE_TEST}` + (cmdData.cmd_inp && ` << ${cmdData.cmd_inp}`);
	...
	cmdData.cmd_ext = 0;
	cmdData.cmd_desc = `NODE_TEE_TEST=1 ${CMD_NODE_TEST} --node` + (cmdData.cmd_inp && ` << ${cmdData.cmd_inp}`);
	...
	cmdData.cmd_ext = 0;
	cmdData.cmd_desc = `NODE_TEE_TEST=1 ${CMD_NODE_TEST} -n` + (cmdData.cmd_inp && ` << ${cmdData.cmd_inp}`);
	...
	cmdData.cmd_ext = 0;
	cmdData.cmd_desc = `NODE_TEE_TEST=1 ${CMD_NODE_TEST} --def` + (cmdData.cmd_inp && ` << ${cmdData.cmd_inp}`);
	...
	cmdData.cmd_ext = 0;
	cmdData.cmd_desc = `NODE_TEE_TEST=1 ${CMD_NODE_TEST} -d` + (cmdData.cmd_inp && ` << ${cmdData.cmd_inp}`);
	...
	cmdData.cmd_ext = 0;
	cmdData.cmd_desc = `NODE_TEE_TEST=1 ${CMD_NODE_TEST} --verbose` + (cmdData.cmd_inp && ` << ${cmdData.cmd_inp}`);
	...
	cmdData.cmd_ext = 0;
	cmdData.cmd_desc = `NODE_TEE_TEST=1 ${CMD_NODE_TEST} -v` + (cmdData.cmd_inp && ` << ${cmdData.cmd_inp}`);
...
`;
	...
	cmdData.cmd_ext = 1;
	cmdData.cmd_desc = `NODE_TEE_TEST=1 ${CMD_NODE_TEST} --xxx` + (cmdData.cmd_inp && ` << ${cmdData.cmd_inp}`);
...
`,
	...
	cmdData.cmd_ext = 1;
	cmdData.cmd_desc = `NODE_TEE_TEST=1 ${CMD_NODE_TEST} -x` + (cmdData.cmd_inp && ` << ${cmdData.cmd_inp}`);
...
cmdData.cmd_err = '';
	...
	// TEST ### - tee tmpDir/testTeeAct-1.pid
	cmdData = {cmd_aux: null};
		...
		fs.open(cmdData.actFile, (err, fd) => {
			if(err) throw err;
				...
				fs.readFile(fd, 'utf8', (err, data) => {
					...
					fs.unlink(cmdData.actFile, (err)=>{if(err) throw err;});
				});
			}
		});
		fs.open(cmdData.expFile, (err, fd) => {
			if(err) throw err;
				...
				fs.readFile(fd, 'utf8', (err, data) => {
					if(err) throw err;
					assert.strictEqual(data, 'test data');
					fs.unlink(cmdData.expFile, (err)=>{if(err) throw err;});
				});
			}
		});
	};
	...
	// TEST ### - tee -a tmpDir/testTeeAct-2.pid
	cmdData = {cmd_aux: null};
	...
	// TEST ### - tee --output-error=warn non-existent/file
	cmdData = {cmd_aux: null};
	...
	// TEST ### - tee --output-error=warn-nopipe non-existent/file
	cmdData = {cmd_aux: null};
	...
	// TEST ### - tee --output-error=exit non-existent/file
	cmdData = {cmd_aux: null};
	...
	// TEST ### - tee --output-error=exit-nopipe non-existent/file
	cmdData = {cmd_aux: null};
	...
	// TEST ### - tee /dev/null non-existent/file /dev/null
	cmdData = {cmd_aux: null};
	...
	// TEST ### - tee --output-error=warn /dev/null non-existent/file /dev/null
	cmdData = {cmd_aux: null};
	...
	// TEST ### - tee --output-error=warn-nopipe /dev/null non-existent/file /dev/null
	cmdData = {cmd_aux: null};
	...
	// TEST ### - tee --output-error=exit /dev/null non-existent/file /dev/null
	cmdData = {cmd_aux: null};
	...
	// TEST ### - tee --output-error=exit-nopipe /dev/null non-existent/file /dev/null
	cmdData = {cmd_aux: null};
	...
	// TEST ### - tee --output-error=warn | noOpCmd
	cmdData = {cmd_aux: null};
	...
	// TEST ### - tee --output-error=warn-nopipe | noOpCmd
	cmdData = {cmd_aux: null};
	...
	// TEST ### - tee --output-error=exit | noOpCmd
	cmdData = {cmd_aux: null};
	...
	// TEST ### - tee --output-error=exit-nopipe | noOpCmd
	cmdData = {cmd_aux: null};
	...
	// TEST ### - tee /dev/null non-existent/file /dev/null | noOpCmd
	cmdData = {cmd_aux: null};
	...
	// TEST ### - tee --output-error=warn /dev/null non-existent/file /dev/null | noOpCmd
	cmdData = {cmd_aux: null};
...
tee: 'standard output': Broken pipe\n`;
	...
	// TEST ### - tee --output-error=warn-nopipe /dev/null non-existent/file /dev/null | noOpCmd
	cmdData = {cmd_aux: null};
	...
	// TEST ### - tee --output-error=exit /dev/null non-existent/file /dev/null | noOpCmd
	cmdData = {cmd_aux: null};
	...
	for(let [suiteDesc, suiteTests] of suites){
		runner.suite(suiteDesc, () => {
			for(let cmdObj of suiteTests){
				runner.test(cmdObj.cmd_desc, {skip: false}, async () => {
...
async function makeTest(obj){
	...
	let [out_act, out_exp, prc_act, prc_exp] = await getCmdOutput(obj.cmd_act, obj.cmd_exp, obj.cmd_inp);
